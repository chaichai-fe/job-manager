...
| 图标     | **lucide-react**                                     |                                                                  |
| 拖拽     | **@dnd-kit**（core / sortable / utilities）          | 项目看板列与卡片的拖拽排序、跨列移动                             |
| 通知     | **sonner**                                           | Toast                                                            |
...
│   │   ├── projects.tsx       # 开发中项目（dnd-kit 拖拽看板，状态列可自定义）
...
│   ├── projects.ts            # projects 增删改查 + moveProject（看板拖拽）
│   ├── project-statuses.ts    # project_statuses 增删改查 + 重排（看板列定义）
...
│   └── app-schema.ts          # 业务表 projects / project_statuses / repo_links / account_entries
│
├── lib/
...
```
projects.tsx (useQuery / useMutation，dnd-kit 看板)
   │
   ├─▼  listProjects / createProject / updateProject / deleteProject / moveProject  (server/projects.ts)
   │       moveProject：拖拽后改 status + 重写目标列 sortOrder
   │
   └─▼  listProjectStatuses / createProjectStatus / updateProjectStatus /
        deleteProjectStatus / reorderProjectStatuses  (server/project-statuses.ts)
            首次访问自动 seed 默认状态列；删除某列时把其下需求迁到剩余列
   │     先 requireUserId() → 再 getDb()
   ▼
Drizzle ORM  ──►  Cloudflare D1 (projects / project_statuses 表，按 userId 过滤)
   │
   ▼  返回数据 → TanStack Query 缓存 → 渲染看板（列 = 状态，卡片 = 需求）
```

> 看板列（状态）不再前端硬编码，而是存在 `project_statuses` 表里、由接口维护：每个用户首次进入 `projects` 页时，`listProjectStatuses` 会自动写入默认四列（待开发 / 开发中 / 测试中 / 已完成），之后可自由增删改、调整顺序与配色。`projects.status` 存的就是某个状态的 `key`。

`links` / `accounts` 两个模块结构与之类似，分别对应 `repo_links`、`account_entries` 表。
...
迁移由 `drizzle-kit` 生成到 `drizzle/migrations/`（`0000_init.sql` 初始建表，`0001_add_project_statuses.sql` 新增看板列表），随 Worker 部署按序 apply 到 D1。
...
业务表（仅逻辑关联 users，未建外键约束，靠 user_id 做隔离）
 projects               (user_id, 索引)   status 引用 project_statuses.key（仅逻辑关联，无外键）
 project_statuses       (user_id, 索引)   看板列定义
...
| `project`     | text 默认 ''        | 项目名                                                          |
| `branch`      | text 默认 ''        | 分支                                                            |
| `status`      | text 默认 `todo`    | 状态值，引用 `project_statuses.key`（看板列由接口维护，非枚举） |
| `note`        | text 默认 ''        | 备注                                                            |

> `status` 历史上是硬编码枚举，现已改为引用 `project_statuses.key` 的自由字符串——看板列（状态）由用户在界面自定义增删，不再写死在代码里。默认值 `todo` 对应首次访问时自动写入的「待开发」列。

**`project_statuses` — 项目状态（看板列）**

| 字段    | 类型              | 说明                                                        |
| ------- | ----------------- | ----------------------------------------------------------- |
| `key`   | text notNull      | 状态值，`projects.status` 引用它（默认列为 `todo` 等可读 key，新建列用 UUID） |
| `label` | text notNull      | 列显示名（如「待开发」）                                    |
| `color` | text 默认 `slate` | 调色板名（前端映射成卡片/列样式，如 rose / sky / violet / emerald） |

> 每个用户首次调用 `listProjectStatuses` 时自动 seed 四个默认列：待开发(todo)/开发中(developing)/测试中(testing)/已完成(done)。删除某列前会把使用该状态的需求迁移到剩余的第一列，且至少保留一列。
